...
from infra.facelivenessbackend.topology import FaceLiveness
from infra.frontend.topology import (
...
class DefaultRflStack(Stack):
    ...
    def __init__(self, scope: Construct, id: str, rfl_stack_name: str, **kwargs) -> None:
        ...
        # Create the FaceLiveness backend
        faceliveness = FaceLiveness(self, 'FaceLiveness', rfl_stack=self)
    ...
    # Setup Frontend~
        feapp = FaceLivenessFrontEnd(
            self,
            "RflWebAPP",
            rfl_stack=self,
            apigateway=faceliveness.api_gateway,
            cognito=cognito
        )

        triggerfeapp = TriggerFrontEndBuild(
            self,
            "RflWebAPPTrigger",
            rfl_stack=self,
            amplifyApp=feapp
        ...
        feappstatus = FaceLivenessFrontEndBuildStatus(
            self,
            "RflWebAPPStatus",
            rfl_stack=self,
            amplifyApp=feapp,
            buildTrigger=triggerfeapp
        )
        feappstatus.node.add_dependency(triggerfeapp)
